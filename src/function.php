...
function getPost($u_id, $p_id)
{
	...
	try {
		$dbh = dbConnect();
		$sql = 'SELECT * FROM posts WHERE user_id = :u_id AND id = :p_id AND delete_flg = 0';
...
function getPostList()
{
	...
	try {
		$dbh = dbConnect();
		$sql = 'SELECT * FROM posts WHERE delete_flg = 0 ORDER BY created_date DESC';
		$data = array();
		debug('SQL:' . $sql);
...
function getPostData($p_id)
{
	...
	try {
		$dbh = dbConnect();
		$sql = 'SELECT * FROM posts WHERE id = :p_id AND delete_flg = 0';
...
function getComment($p_id)
{
	...
	try {
		$dbh = dbConnect();
		$sql = 'SELECT * FROM comments WHERE post_id = :p_id AND delete_flg = 0 ORDER BY created_date DESC';
		$data = array(':p_id' => $p_id);
		// クエリ実行
		$stmt = queryPost($dbh, $sql, $data);
...
function getGood($p_id)
{
	...
	try {
		$dbh = dbConnect();
		$sql = 'SELECT * FROM benches WHERE post_id = :p_id';
		$data = array(':p_id' => $p_id);
		// クエリ実行
		$stmt = queryPost($dbh, $sql, $data);
...
function isGood($u_id, $p_id)
{
	...
	try {
		$dbh = dbConnect();
		$sql = 'SELECT * FROM benches WHERE post_id = :p_id AND user_id = :u_id';
		$data = array(':u_id' => $u_id, ':p_id' => $p_id);
		// クエリ実行
		$stmt = queryPost($dbh, $sql, $data);
...
function getUserPostList($u_id)
{
	...
	try {
		$dbh = dbConnect();
		$sql = 'SELECT * FROM posts WHERE user_id = :u_id AND delete_flg = 0 ORDER BY created_date DESC';
		$data = array(':u_id' => $u_id);
		// クエリ実行
		$stmt = queryPost($dbh, $sql, $data);
...
function getUserGoodPostList($u_id)
{
	...
	try {
		$dbh = dbConnect();
		$sql = 'SELECT p.id, p.contents, p.post_img, p.user_id, p.created_date, p.delete_flg FROM posts AS p INNER JOIN good AS g ON p.id = g.post_id WHERE g.user_id = :u_id AND p.delete_flg = 0 ORDER BY created_date DESC';
		$data = array(':u_id' => $u_id);
		// クエリ実行
		$stmt = queryPost($dbh, $sql, $data);
